...
// Funkcja CRC-8 (XOR / Polynomial 0x07) przeliczana dla pierwszych 11 bajtów
...
bool parse_packet(const uint8_t *raw_buf, volatile Cluster *cluster) {
    ...
    // 2. Walidacja CRC (liczona od bajtu 0 do 10, czyli pierwsze 10 bajtów)
    uint8_t computed_crc = calculate_crc8(raw_buf, 11);
    ...
    // 3. Przepisanie i rekonstrukcja wartości
	// Numer ramki
    cluster->last_pkt_num = (uint16_t)((pkt->pkt_num_msb << 8) | pkt->pkt_num_lsb);
